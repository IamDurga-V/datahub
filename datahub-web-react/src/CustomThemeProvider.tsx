...
// import defaultThemeConfig from '@conf/theme/theme_dark.config.json';
...
// This is the key change. We are importing the dark theme directly.
import defaultThemeConfig from '@conf/theme/theme_dark.config.json';
import { Theme } from '@conf/theme/types';
import { CustomThemeContext } from '@src/customThemeContext';
...
const CustomThemeProvider = ({ children }: Props) => {
    // We are initializing the theme state with the dark theme and removing the dynamic loading logic.
    const [currentTheme] = useState<Theme>(defaultThemeConfig);

    return (
        <CustomThemeContext.Provider value={{ theme: currentTheme, updateTheme: () => {} }}>
            <ThemeProvider theme={currentTheme}>{children}</ThemeProvider>
        </CustomThemeContext.Provider>
    );
};
